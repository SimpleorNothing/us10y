...
# FedWatch 확률 인용 규칙 (반드시 준수 — 과거에 수치를 지어내거나 날마다 모순되게 쓰는 오류가 있었음)
- CME FedWatch 인하/동결/인상 확률(%)은 반드시 web_search로 '오늘자' CME FedWatch Tool 값을 직접 확인한 경우에만 인용한다. 확인하지 못하면 수치 대신 '확인 안 됨'으로 적고 함의금리 변화만 정성적으로 서술한다.
- 인용 시 기준 회의(예: 6월 FOMC, 12월 FOMC)와 as-of 날짜를 함께 명시한다. 예: "6월 동결 98.6%, 12월 인상 ~51% (CME 6/2)".
- '인상 확률'은 현재 목표금리 구간(함의금리로 추정)보다 높은 모든 구간 확률의 합, '인하 확률'은 낮은 모든 구간의 합으로 일관되게 정의한다. 한 회의를 '인하' 기준으로, 다른 날 같은 회의를 '인상' 기준으로 뒤바꿔 서술하지 말 것.
- 함의금리가 거의 변하지 않았는데(예: ±2bp 이내) 전날 대비 FedWatch 서술 방향(인하↔인상)이 급변하면 잘못 본 것이니 재확인한다.
...
4. 다른 IB의 금리 전망 업데이트
5. CME FedWatch 확률 — 다음 FOMC(가장 가까운 회의)와 연말(12월) FOMC의 인하/동결/인상 확률을 web_search로 오늘자 값을 직접 확인. 'Fed Funds 30D' key_driver에는 확인한 회의·확률·as-of 날짜를 함께 적고(위 'FedWatch 확률 인용 규칙' 준수), 확인 못 하면 수치를 지어내지 말 것
